...
[![Commits](https://img.shields.io/badge/Commits-21-1e88e5?style=flat-square)](https://github.com/imztz/llm-journey/commits)
<!-- AUTO:PROGRESS:END -->
...
├── scripts/            # README 自动化(status.json 阶段表 + update_readme.py)
├── .github/workflows/  # 每日定时更新 README 动态片段(Status / Day / 进度徽章)
└── README.md           # 你在这里
```
...
<details>
<summary><b>⚡ README 自动化机制</b>(点击展开)</summary>

本 README 有三处动态内容由 GitHub Actions 自动维护,**不需要手动改**:

| 内容 | 更新规则 | 标记位置 |
|---|---|---|
| Status 徽章 | 按 `scripts/status.json` 里的阶段日期表,由当前日期推断所处阶段 | 头部 `AUTO:STATUS` |
| Day X of N 计数器 | `(今天 − 2026-08-21) + 1`,总数按配置的结束日计算 | 尾部 `AUTO:DAY` |
| 进度徽章(Commits 数 / Actions 运行状态) | GitHub REST API 实时统计本仓库提交数 | 头部 `AUTO:PROGRESS` |

**运行时机**(`.github/workflows/update-readme.yml`):

- 每日北京时间 00:23 定时触发(cron `23 16 * * *` UTC)
- 每次 push 到 `main`(README 自身的自动提交除外,防止循环)
- 手动触发:Actions → *Update README* → *Run workflow*

**手动覆盖 Status**(比如阶段实际进度与日期表不符时):

编辑 `scripts/status.json`,把 `override_status` 设为任意字符串(如 `"Phase 1 · GRPO 冲刺中"`),提交即可——脚本会**优先使用它**,日期推断被挂起;改回 `null` 即恢复全自动。

**调整阶段划分 / 总天数:** 同样只改 `scripts/status.json` 的 `phases` 表和 `start` / `end` 字段,README 无需动。

**仍然手动维护的部分:** 里程碑看板、实验记录表、博客表——按月度复盘节奏更新。打勾这个动作本身是复盘的一部分,不自动化。

**技术选型(官方优先):** GitHub Actions(cron 调度)+ `GITHUB_TOKEN`(内置 token 调 REST API,**零密钥配置**)+ `github-actions[bot]` 身份提交;shields.io 只负责把数字渲染成图片,不存任何数据。本地也能跑:`python scripts/update_readme.py`(无 token 时自动跳过进度徽章,只更新日期相关内容)。
